...
document.addEventListener('DOMContentLoaded', () => {
    ...
    const setupQtyLogic = () => {
        document.querySelectorAll('.qty-minus').forEach(btn => {
            btn.addEventListener('click', (e) => {
                ...
                if (val > 0) {
                    input.value = val - 1;
                    if (input.value == 0) {
                        card.classList.remove('border-wood-accent', 'ring-1', 'ring-wood-accent', 'bg-wood-accent/5', 'dark:bg-wood-accent/10');
                        card.querySelector('.card-icon').classList.remove('text-wood-accent');
                    }
        ...
        document.querySelectorAll('.qty-plus').forEach(btn => {
            btn.addEventListener('click', (e) => {
                ...
                let val = parseInt(input.value) || 0;
                input.value = val + 1;
                card.classList.add('border-wood-accent', 'ring-1', 'ring-wood-accent', 'bg-wood-accent/5', 'dark:bg-wood-accent/10');
    ...
    const updateMainTablePrices = (isPhp) => {
        ...
        if (isPhp) {
            ...
            labelUSDT?.classList.remove('text-gray-900', 'dark:text-white', 'font-bold');
        } else {
            ...
            labelPhp?.classList.remove('text-gray-900', 'dark:text-white', 'font-bold');
        }
    };

    const calculateTotals = () => {
        isPhp = currencyToggle?.classList.contains('bg-wood-accent') ?? true;
        let otPhp = 0, otUSDT = 0, mPhp = 0, mUSDT = 0;
        ...
        currentCheckboxes.forEach(cb => {
            if (cb.checked) {
                ...
                if (cb.dataset.type === 'onetime') { otPhp += pPhp; otUSDT += pUSDT; }
                else { mPhp += pPhp; mUSDT += pUSDT; }
            }
        });
        ...
        if (onetimeTotalEl) onetimeTotalEl.innerText = formatPrice(isPhp ? otPhp : otUSDT, isPhp);
        if (monthlyTotalEl) monthlyTotalEl.innerHTML = formatPrice(isPhp ? mPhp : mUSDT, isPhp) + '<span class="text-xs text-white/70">/mo</span>';
        ...
        document.querySelectorAll('.price-display').forEach(display => {
            ...
            const isM = (display.dataset.basePhp && display.closest('.qty-card')?.querySelector('.quote-select')) || display.innerText.includes('/mo');
            display.innerText = formatPrice(isPhp ? php : usdt, isPhp) + (isM ? '/mo' : '');
        });
    ...
    const syncModalCurrencyUI = (isPhp) => {
        ...
        modalCurrencyBtns.forEach(btn => {
            if ((btn.dataset.currency === 'PHP' && isPhp) || (btn.dataset.currency === 'USDT' && !isPhp)) {
                btn.classList.add('bg-wood-accent', 'text-white');
                btn.classList.remove('text-gray-500', 'hover:text-gray-700', 'dark:hover:text-gray-300');
            } else {
    ...
    const saveState = () => {
        ...
        const state = {
            ...
            qtys: Array.from(document.querySelectorAll('.quote-qty-input')).map(i => parseInt(i.value) || 0)
        };
    ...
    const loadState = () => {
        try {
            ...
            if (state.email) document.getElementById('client-email').value = state.email;
            if (state.details) document.getElementById('client-details').value = state.details;

            const currentCheckboxes = document.querySelectorAll('.quote-item');
            state.checkboxes?.forEach((c, i) => { if (currentCheckboxes[i]) currentCheckboxes[i].checked = c; });

            const currentSelects = document.querySelectorAll('.quote-select');
            state.selects?.forEach((s, i) => { if (currentSelects[i]) currentSelects[i].selectedIndex = s; });
    ...
    // --- Event Listeners ---
    document.querySelectorAll('.quote-item').forEach(cb => cb.addEventListener('change', () => { calculateTotals(); saveState(); }));
    document.querySelectorAll('.quote-select').forEach(s => s.addEventListener('change', () => { calculateTotals(); saveState(); }));
    ['client-name', 'client-email', 'client-details'].forEach(id => document.getElementById(id)?.addEventListener('input', saveState));
    ...
    form.addEventListener('submit', async (e) => {
        ...
        document.querySelectorAll('.quote-item').forEach(cb => {
            if (cb.checked) {
                const label = cb.closest('label').querySelector('.item-label').innerText;
                selectedItems.push({ item: label, price: parseFloat(cb.dataset.pricePhp) });
            }
        });
        ...
        document.querySelectorAll('.quote-qty-input').forEach(inp => {
            ...
            if (qty > 0) {
                const label = inp.closest('.qty-card').querySelector('.item-label').innerText;
                const price = parseFloat(inp.dataset.pricePhp);
                selectedItems.push({ item: `${label} (x${qty})`, price: price * qty, qty });
            }
        });
        ...
        document.querySelectorAll('.quote-select').forEach(s => {
            ...
            if (s.selectedIndex > 0) {
                selectedItems.push({ item: opt.innerText.split('(')[0].trim(), price: parseFloat(opt.value), type: 'monthly' });
            }
        });
        ...
        try {
            ...
            feedbackEl.className = "mt-3 text-sm text-center font-bold text-green-400 p-3";
            ...
            feedbackEl.className = "mt-3 text-sm text-center font-bold text-red-400 p-3";
            feedbackEl.classList.remove('hidden');
        } finally {
