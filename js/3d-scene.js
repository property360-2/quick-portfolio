...
if (!container) {
    console.warn("3D Scene: Container #cat-canvas-container not found. Skipping initialization.");
} else {
    ...
    const buildProceduralCat = () => {
        ...
        const muzzleMaterial = new THREE.MeshStandardMaterial({ color: 0xA1887F, roughness: 0.7 });
    ...
    outlineCat.traverse((node) => {
        if (node.isMesh) {
            ...
            if (node.geometry.type === 'CylinderGeometry' || node.material.type === 'MeshBasicMaterial') {
    ...
    function animate() {
        ...
        catGroup.position.y = catGroup.userData.startY + Math.sin(catGroup.userData.time * 1.5) * 0.08;
